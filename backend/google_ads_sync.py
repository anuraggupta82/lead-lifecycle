...
from database import get_all_leads, upsert_lead
...
def _fetch_keyword_costs(client, customer_id: str) -> dict:
    ...
    logger.info("Querying keyword_view for cost data...")
    ...
    try:
        ...
        for row in response:
            ...
            clicks = row.metrics.clicks or 1
            cost_dollars = (row.metrics.cost_micros or 0) / 1_000_000.0
            avg_cpc = cost_dollars / clicks if clicks > 0 else 0

            keyword_costs[keyword] = avg_cpc

        logger.info(f"Found cost data for {len(keyword_costs)} keywords")

    except Exception as e:
        logger.error(f"keyword_view query failed: {e}")

    return keyword_costs
...
def sync_gclids_to_keywords(days_back: int = 7) -> dict:
    ...
    # Pass 2: Get keyword → cost mapping
    keyword_costs = _fetch_keyword_costs(client, customer_id)
